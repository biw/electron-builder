...
const projectModuleResolver_1 = require("../util/projectModuleResolver");
...
async function getElectronVersionFromInstalled(projectDir) {
    for (const name of electronPackages) {
        try {
            ...
            const packageJsonPath = (0, projectModuleResolver_1.resolveFromProject)({
                projectDir,
                moduleSpecifier: `${name}/package.json`,
                optional: true,
            });
            if (packageJsonPath) {
                return (await (0, fs_extra_1.readJson)(packageJsonPath)).version;
            }
...
async function getElectronPackage(projectDir) {
    for (const name of electronPackages) {
        try {
            ...
            if (packageJsonPath) {
                return await (0, fs_extra_1.readJson)(packageJsonPath);
            }
            // Fallback to direct path check
