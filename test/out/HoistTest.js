...
const hoist_1 = require("app-builder-lib/out/node-module-collector/hoist");
const vitest_1 = require("vitest");
const toTree = (obj, key = `.`, nodes = new Map()) => {
    ...
    if (!node) {
        node = {
            ...
            peerNames: new Set((obj[key] || {}).peerNames || []),
...
describe(`hoist`, () => {
    it(`should do very basic hoisting`, () => {
        ...
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(2);
    });
    ...
    it(`should support cyclic peer dependencies`, () => {
        ...
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(3);
    });
    ...
    it(`should perform deep hoisting`, () => {
        ...
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(4);
    });
    ...
    it(`should honor package popularity considering number of all references over number of references by peers`, () => {
        ...
        const result = (0, hoist_1.hoist)(tree, { check: true });
        (0, vitest_1.expect)(getTreeHeight(result)).toEqual(3);
        const topLevelDeps = [...result.dependencies];
        const hoistedZ = topLevelDeps.find(x => x.name === `Z`);
        (0, vitest_1.expect)(hoistedZ.references).toContain(`X`);
        (0, vitest_1.expect)(hoistedZ.references).not.toContain(`Y`);
        const D = topLevelDeps.find(x => x.name === `D`);
        const dDeps = [...D.dependencies];
        (0, vitest_1.expect)(dDeps.length).toEqual(2);
        const nestedZ = dDeps.find(x => x.name === `Z`);
        (0, vitest_1.expect)(nestedZ.references).not.toContain(`X`);
        (0, vitest_1.expect)(nestedZ.references).toContain(`Y`);
    });
    ...
    it(`should keep peer dependency promise for the case where the package with same ident is a dependency of parent node`, () => {
        ...
        const hoistedTree = (0, hoist_1.hoist)(toTree(tree), { check: true });
        const [A] = Array.from(hoistedTree.dependencies).filter(x => x.name === `A`);
        (0, vitest_1.expect)(Array.from(A.dependencies).filter(x => x.name === `B`)).toBeDefined();
    });
    ...
    it(`should not hoist packages past hoist boundary`, () => {
        ...
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true, hoistingLimits }))).toEqual(3);
    });
    ...
    it(`should hoist direct workspace dependencies into non-root workspace`, () => {
        ...
        const tree = {
            ".": { dependencies: [`W1(w)`, `W3`, `A@Z`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
            "W1(w)": { dependencies: [`W2(w)`, `A@Y`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
            "W2(w)": { dependencies: [`W3(w)`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
            "W3(w)": { dependencies: [`A@X`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
        };
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(4);
    });
    it(`should hoist dependencies to the top from workspaces that have no hoist borders given there is workspace with hoist borders`, () => {
        ...
        //   -> B@Y
        const tree = {
            ".": { dependencies: [`W1(w)`, `W2(w)`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
            "W1(w)": { dependencies: [`A@X`, `B@X`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
            "A@X": { dependencies: [`B@X`], peerNames: [`B`] },
            "A@Y": { dependencies: [`B@Y`], peerNames: [`B`] },
            "W2(w)": { dependencies: [`A@Y`, `B@Y`], dependencyKind: hoist_1.HoisterDependencyKind.WORKSPACE },
        };
        const hoistingLimits = new Map([[`.@`, new Set([`W1(w)`])]]);
        const hoistedTree = (0, hoist_1.hoist)(toTree(tree), { check: true, hoistingLimits });
        const W2 = Array.from(Array.from(hoistedTree.dependencies).filter(x => x.name === `W2(w)`)[0].dependencies);
        (0, vitest_1.expect)(W2).toEqual([]);
    });
    ...
    it(`should not hoist portal with unhoistable dependencies`, () => {
        const tree = {
            ".": { dependencies: [`P1`, `B@Y`] },
            P1: { dependencies: [`P2`], dependencyKind: hoist_1.HoisterDependencyKind.EXTERNAL_SOFT_LINK },
            P2: { dependencies: [`B@X`], dependencyKind: hoist_1.HoisterDependencyKind.EXTERNAL_SOFT_LINK },
        };
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(3);
    });
    it(`should hoist nested portals with hoisted dependencies`, () => {
        const tree = {
            ".": { dependencies: [`P1`, `B@X`] },
            P1: { dependencies: [`P2`, `B@X`], dependencyKind: hoist_1.HoisterDependencyKind.EXTERNAL_SOFT_LINK },
            P2: { dependencies: [`P3`, `B@X`], dependencyKind: hoist_1.HoisterDependencyKind.EXTERNAL_SOFT_LINK },
            P3: { dependencies: [`B@X`], dependencyKind: hoist_1.HoisterDependencyKind.EXTERNAL_SOFT_LINK },
        };
        (0, vitest_1.expect)(getTreeHeight((0, hoist_1.hoist)(toTree(tree), { check: true }))).toEqual(2);
    });
...
//# sourceMappingURL=HoistTest.js.map