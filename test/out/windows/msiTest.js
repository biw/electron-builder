...
describe("msi", { sequential: true }, () => {
    test.ifDevOrWinCi("msi", ({ expect }) => (0, packTester_1.app)(expect, {
    ...
    test.ifDevOrWinCi("msi no asar", ({ expect }) => (0, packTester_1.app)(expect, {
        targets: electron_builder_1.Platform.WINDOWS.createTarget("msi"),
        config: {
            appId: "build.electron.test.msi.oneClick.perMachine",
            extraMetadata: {
    ...
    test.ifDevOrWinCi("per-user", ({ expect }) => (0, packTester_1.app)(expect, {
        targets: electron_builder_1.Platform.WINDOWS.createTarget("msi"),
        config: {
    ...
    test.ifDevOrWinCi("wix args", ({ expect }) => (0, packTester_1.app)(expect, {
        targets: electron_builder_1.Platform.WINDOWS.createTarget("msi"),
        config: {
            ...
            msiProjectCreated: async (path) => {
                await fs.promises.writeFile(path, (await fs.promises.readFile(path, "utf8")).replace("</Product>", `<util:CloseApplication xmlns:util="http://wixtoolset.org/schemas/v4/wxs/util"
              PromptToContinue="no"
    ...
    test.skip("assisted", ({ expect }) => (0, packTester_1.app)(expect, {
        targets: electron_builder_1.Platform.WINDOWS.createTarget("msi"),
        config: {
